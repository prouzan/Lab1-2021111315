...
def read_text_file(file_path_in_read_text_file):
    """Module providing a function reading text file."""
    try:
        with open(file_path_in_read_text_file, 'r', encoding='utf-8') as file:
            text = file.read()
        return text
    except FileNotFoundError:
        print(f"File not found: {file_path_in_read_text_file}")
        sys.exit(1)


def preprocess_text(text):
    """Module providing a function preprocess text file."""
...
class Graph():
    """Module providing a function handle user command."""
    words = []
    maps = []
    nodes = []
    graph = []
    ...
    def build_node_map(self):
        """Module providing a function build node map."""
    ...
    def init_graph(self):
        """Module providing a function init graph."""
    ...
    def build_graph(self):
        """Module providing a function build graph."""
    ...
    def find_bridge_word(self, word1, word2, gen=False):
        """Module providing a function find bridge word."""
        flag1 = 0
        ...
        if gen is False:
            if len_of_bridge_words >= 3:
                print(f"The bridge words from \"{word1}\" to \"{word2}\" \
                      are:{', '.join(bridge_words[:-2])}, \
                      {bridge_words[-2]} and {bridge_words[-1]}")
            elif len_of_bridge_words == 2:
                print(f"The bridge words from \"{word1}\" to \"{word2}\" are: \
                      {bridge_words[-2]} and {bridge_words[-1]}")
            elif len_of_bridge_words == 1:
                print(f"The bridge word from \"{word1}\" to \"{word2}\" is: \
                      {bridge_words[0]}")
            else:
    ...
    def generateNewText(self, newwords):
        """Module providing a function generate New Text."""
        result_words = []
    ...
    def map(self, word2):
        """Module providing a function maping."""
    ...
    def find_shortest_path(self, word1, word2=None, info=True):
        """Module providing a function find shortest path."""
        map1 = self.map(word1)
        map2 = self.map(word2)
        ...
        if word2 is not None:
            ...
            if array[map2] != 0 and info is True:
    ...
    def all_shortest(self, word1, word2):
        """Module providing a function find all shortest path."""
        map1 = self.map(word1)
        map2 = self.map(word2)
        local_graph = copy.deepcopy(self.graph)
        shortest_paths = []
    ...
    def random_walk(self):
        """Module providing a function random walk."""
        ...
        for index, line in enumerate(path):
            if index == 0:
                real_path.append(self.nodes[line[0]])
            real_path.append(self.nodes[line[1]])
        print("Random walk:")
        if real_path:
    ...
    def draw_graph(self, highlight_path=None, save_path=None):
        """Module providing a function draw_graph."""
        gra = nx.DiGraph()
        for i in range(self.size):
            gra.add_node(self.nodes[i])
        for i in range(self.size):
            for j in range(self.size):
                if self.graph[i][j] > 0:
                    gra.add_edge(self.nodes[i], self.nodes[j], weight=self.graph[i][j])
        pos = {node: (index, 0) for index, node in enumerate(self.nodes)}
        plt.figure(figsize=(18, 12))
        nx.draw(gra, pos, with_labels=True, node_size=3000, node_color="skyblue", font_size=12, \
                font_color="black", font_weight="bold", edge_color="gray", arrowsize=20, \
                    connectionstyle="arc3,rad=0.2")
        edge_labels = {(self.nodes[i], self.nodes[j]): self.graph[i][j] for i in \
                       range(len(self.graph)) for j in range(self.size) \
                        if self.graph[i][j] > 0}
        for (n1, n2), label in edge_labels.items():
            ...
            y_label = sig * ((y1 + y2) / 2 + 0.1 * x_diff)  
            # Adjust y_label dynamically based on x_diff
            plt.text(x_label, y_label, label, fontsize=10, ha='center', va='center', color='red')
        if highlight_path:
            edges = [(self.nodes[highlight_path[i]], self.nodes[highlight_path[i+1]]) \
                     for i in range(len(highlight_path)-1)]
            nx.draw_networkx_edges(gra, pos, edgelist=edges, edge_color='red', width=2.5, \
...
def main(file_path_in_main):
    """Module providing a function read user command."""
    text = read_text_file(file_path_in_main)
    ...
    while True:
        ...
        if command == '1':
            ...
            word1 = input("Enter the first word (or 'exit' to quit): ").lower()
            if word1 != 'exit':
                word2 = input("Enter the second word(if there is no \
                              second word please input 0): ").lower()
                if word2 != '0': 
